...
async function callGemini(prompt, env) {
  const r = await llmCall({ prompt, temperature: 0.2, maxTokens: 2500 });
...
async function main() {
  ...
  await journal("brand-steward", `brand health scored for ${results.length} tenants (${drifts.length} drift alerts)`);
}
